...
import static com.ib.client.Types.SecType.*
import static lv.sergluka.ib_client.IbClient.BarSize.*
import static org.assertj.core.api.Assertions.assertThat

class IbClientTest extends Specification {

    public static final String ACCOUNT = "DU229550"
    ...
    void setup() {
        ...
        client.cancelAll().timeout(Duration.ofSeconds(10), Mono.empty()).block()
        closeAllPositions()
    }
    ...
    def closePosition(IbPosition position) {
        ...
        def contractDetails = client.reqContractDetails(position.contract).take(1).blockLast()
...
//            client.cancelAll().block()()
    ...
    def "Requesting matching symbols for FB should return valid list"() {
        ...
        StepVerifier.create(client.reqMatchingSymbols("FB"))
                .assertNext { IbContractDescription description ->
                    ...
                    assert description.contract.secType == STK
                ...
                .assertNext { IbContractDescription description ->
                    ...
                    assert description.derivativeSecTypes == [CFD, OPT, IOPT, WAR, FUT, BAG]
                }
                .expectNextCount(15)
                .verifyComplete()
    }
    ...
    def "Request cannot be duplicated"() {
        ...
        expect:
        StepVerifier.create(Flux.merge(2, client.reqOpenOrders(), client.reqOpenOrders()).then())
